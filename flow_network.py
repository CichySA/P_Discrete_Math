...
def jain(x):
    ...
    if denom <= 1e-9:
        return 0.0
    return float(np.sum(x) ** 2 / (len(x) * denom))
...
def plot_flow_graph(
    ...
    if show_saturation:
        ...
        cbar.set_label("Arc Saturation (Flow/Capacity, Full = Purple)", rotation=270, labelpad=15)
...
def plot_network(G, flow_dict, pos=None, title="Flow Network"):
    plot_flow_graph(
        G,
        flow_dict,
        pos=pos,
