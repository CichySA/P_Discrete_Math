...
\section{General overview on flow networks}
...
\begin{definition}[Flow conservation / Kirchoff's Law]
Description
\[
    \sum_{(u,v)\in E} f(u,v)
    \;=\;
...
\subsubsection{Algorithms} run mostly in polynomial time and give optimal solutions
\begin{table}[h]
\centering
\begin{tabular}{|l|c|l|}
\hline
...
\end{table}
\subsubsection{Maximum flow} is a special case used in other network problem


\newpage
\section{Szymon Part : Flow networks, duality, fairness}
...
\subsection{Lagrangian and Karush–Kuhn–Tucker conditions}
...
The trade-offs between solution could have many definitions, like the flows on $e_i$ vs $e_j$, or trade-offs between optimization objectives, like total flow versus \texit{fairness}, however we define it, in our case.
...
Here $w_k \ge 0$ are optional per-sink weights. When $w_k = 1$ for all $k$, the weights are \textit{uniform}.
...
objective = cvxpy.Maximize(cvxpy.sum(cvxpy.log(U + 1e-9)))
\end{minted}

We sweep $\alpha \in \{0, 0.5, 1, 2, 5\}$ and record per-sink utilisation $S_k/D_k$ and total throughput. \figref{alpha_sweep_uniform} shows the results under uniform costs ($w_k = 1$).

\begin{figure}[H]
    \centering
    \includegraphics[width=0.85\linewidth]{Project/img/alpha_sweep_uniform.png}
    \caption{\textbf{$\alpha$-fairness spectrum under uniform delivery costs.} Left: per-hospital utilisation $S_k/D_k$ across $\alpha$ values. Right: total throughput vs.\ $\alpha$. As $\alpha$ increases, utilisation across hospitals becomes more equal but total throughput decreases. At $\alpha = 1$ (proportional fairness), the allocation is already balanced; higher $\alpha$ redistributes flow away from high-demand hospitals without meaningfully helping underserved ones.}
    \label{fig:alpha_sweep_uniform}
\end{figure}

Under uniform costs, $\alpha = 1$ achieves proportional fairness --- the Nash bargaining solution --- where any marginal improvement for one hospital comes at a proportionally larger loss for others. Increasing $\alpha$ beyond 1 further equalises utilisation but reduces total throughput. Crucially, this does not help underserved hospitals: it diverts flow from hospitals with high demand to those with low demand, even though the low-demand hospitals already receive adequate supply at $\alpha = 1$. In the uniform-cost setting, $\alpha = 1$ is the natural sweet spot.

\paragraph{Weighted delivery costs.} In practice, hospitals at different distances from the depot face different transportation costs. We introduce per-sink weights $w_k$ to model this asymmetry. Under $\alpha = 1$, the optimiser may favour sinks with higher weights (cheaper to serve), creating cost-driven imbalance.

\figref{alpha_sweep_weighted} compares the utilisation profiles under uniform and weighted costs. With weights $w = (1.0, 0.4, 0.2)$ --- representing progressively more expensive deliveries --- the $\alpha = 1$ solution heavily favours $t_1$. Increasing $\alpha$ overcomes this cost pressure by forcing the optimiser to redistribute flow toward underserved (expensive) sinks.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.85\linewidth]{Project/img/alpha_sweep_weighted.png}
    \caption{\textbf{Fairness under cost asymmetry.} Left: uniform costs ($w_k = 1$) --- $\alpha = 1$ already balances the allocation. Right: weighted costs ($w = 1.0, 0.4, 0.2$) --- $\alpha = 1$ favours the cheapest sink; increasing $\alpha$ redistributes flow toward expensive, underserved hospitals.}
    \label{fig:alpha_sweep_weighted}
\end{figure}

The throughput comparison in \figref{cost_of_fairness} shows that the \textit{cost of fairness} --- the throughput lost to achieve egalitarianism --- is higher under weighted costs. This is the price of overcoming cost-driven bias.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.6\linewidth]{Project/img/cost_of_fairness_comparison.png}
    \caption{\textbf{Cost of fairness: uniform vs.\ weighted delivery costs.} Under uniform costs, throughput decreases smoothly with $\alpha$. Under weighted costs, a larger throughput sacrifice is needed to overcome cost-driven bias toward cheaper sinks.}
    \label{fig:cost_of_fairness}
\end{figure}
...
\]
by solving a sequence of constrained problems: (a) fix a target throughput and maximise fairness, and (b) fix a target Jain index and maximise throughput. Each point on the frontier represents a different efficiency-equity trade-off.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.65\linewidth]{Project/img/pareto_frontier.png}
    \caption{\textbf{Pareto frontier: efficiency vs.\ fairness.} The frontier traces all achievable (throughput, Jain index) pairs. The utilitarian optimum ($\alpha = 0$) achieves maximum throughput but lowest fairness; the egalitarian solution achieves maximum fairness at the cost of throughput. The Price of Fairness is the horizontal gap between these extremes.}
...
\[
\mathcal{L}(f, \lambda, \mu) = \sum_k \log\!\left(\frac{S_k}{D_k}\right)
+ \mu^\top A_{\text{cons}} f
+ \lambda^\top (c - f),
\]
where $\lambda_e \ge 0$ are the dual variables for the capacity constraints and $\mu_u$ are the node potentials from flow conservation. The stationarity condition gives:
\[
\frac{\partial \mathcal{L}}{\partial f_e} = 0
\quad\Longrightarrow\quad
\sum_{k:\, e \text{ reaches } t_k} \frac{D_k}{S_k} + (A_{\text{cons}}^\top \mu)_e - \lambda_e = 0.
\]
...
Complementary slackness $\lambda_e (c_e - f_e) = 0$ guarantees that only fully saturated edges carry a positive congestion toll. This connects the fairness dual to the max-flow dual discussed earlier: while the max-flow dual has integral $\{0,1\}$ solutions (due to total unimodularity), the fairness dual produces continuous shadow prices that reflect the \textit{marginal value of capacity} in terms of utility rather than raw flow.

\section{Maxime Part : Study of sensitivity}

\subsection{Context}

Based on the second presentation on the topic "Shortest path algorithms in weighted graphs", we had use and improve the graph that represent Wroclaw city. The \figref{wroclaw_graph} presents the final graph that we will use to run our experiments.
...
Regarding the capacity scale, we apply the following rules for the tree type of category:

\begin{itemize}
    \item weight of $50$ and more when it involves highwyas or major arteries such as the A4 or Legnicka ;
    \item weight between $20$ and $40$ for standard city avenues such as the \textit{Plac Grunwaldzki} ;
    \item weight between $5$ and $15$ for the small streets, historic bridges and residental areas.
\end{itemize}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.9\linewidth]{img/wroclaw_graph.png}
    \caption{Wroclaw graph uses as a context for our work.}
...
First, we will present the mathemtical background to understand and solve this problem \ref{sec:1_theoretical_review}, then we will present a naive method to find those edges \ref{sec:1_naive_method}. Finally, we will present a optimized method based on a powerful mathelatical result \ref{sec:1_optimized_method}.
...
Mathematically, we maximize the total flow $v$ subject to the Kirchhoff's law \eqref{eq:kirchhoff_lax} and the capacity constraint \eqref{eq:capacity_constraint}.

The Kirchhoff's law states that for any node $k \in V \backslash \{s,t\}$, the sum of incoming lows equals the sum of outgoing flows:

\begin{equation}\label{eq:kirchhoff_lax}
...
If you increase the capacity $c_{ij}$ of an edge by a value $\delta$, the total flow $v$ will only increase if the edge $(i,j)$ belongs to the limiting minimum cut.
...
On the one hand, if the shadow is strictly positive, the edge is a bottleneck (in the minimum cut). Increasing $c_{ij}$ increases the total flow. On the other hand, if the shadow price is zero, the edge's capacity is not saturated, or saturating it does not limit the overall flow. Increasing $c_{ij}$ is a waste of epidemiological resources.
...
However, the naive method is not optimal as we need to test every single edge. That is the reason why we can apply the \emph{Max-Flow Min-Cut Theorem} that states that if you increase the capacity $c_{ij}$ of and edge by a certain value $\delta$, the total flow will only increase if the edge belogs to the limiting minimum cut.

Therefore, we do not need to test every single edge, we only need to find and test the edges that are in limiting minimum cut. Hence, we are able to improbe our method into an optimized method (\mylistref{1_optimized_method}).
...
As the naive method test every single edge of the graph, by noting the total number of edges $\abs{E}$ (cardinal), this method requires exactly $1+\abs{E}$ number of iterations. However, the optimized method test all edges within the min-cut (\tableref{nb_iterations_methods_1}).
...
In order to compare these two methods, we will runder a benchmark. First, we will generate a random Erdős-Rényi oriented graph and we will set a random capacity between $1$ and $15$. We make sure to check if there is a path from the source to the sink because, otherwise, the total flow is null and the test is not relevant.

Then we can track the evolution of computation time in function of the size if the random generated graph (\figref{benchmark_1_all}). On the normal scale, we can see how efficient the optimized method is comparing to the naive method. As the graph's size are all in the same order, the cardinal of the min-cut is similar which explains the low-growing function that we observe.
...
\begin{figure}[H]
    ...
    \end{subfigure}
    \caption{\textbf{Comparison between the naive and the optimized method.} The \figref{benchmark_1} present the performance with the number of nodes in the graph and the \figref{benchmark_1_log} present the same evolution using a $\log$ scale.}
...
Let's consider the graph \figref{base_toy_graph_1}. We can see that the inital max flow is 11. We observe that, if we add $+10$ to a non-bottleneck edge (between $S$ and $A$), then the new flow is also $11$. Therefore, adding capacity to a non-bottleneck edge is absolutely irrelevant.
...
Let's now consider the case where we have multiple min-cut edge. In order to study the impact on the total flow, let's consider the graph illustrated in \figref{base_toy_graph_2}.

The initial max flow is $10$ and we will upgrade on min cut (the edge between nodes $S$ and $A$) by $+5$. We observe then that the new flow is the same, the flow didn't increase because we are still constrained by the edge between $A$ and $T$.
...
Our goal is now to measure the flow loss if a ridge is completely removed (or if its capacity is reduced to zero). This can be the case if, for instance, a road is removed or if an unexpected issue occurs.

First, we will present the mathematical background to understand and solve this problem \ref{sec:2_theoretical_review}, then we will present a naive method to find those edeges \ref{sec:2_naive_method}. Finally, we will present a optimized method based on a powerful mathematical result \ref{sec:2_optimized_method}.
...
First, the naive method can be considered: for every single edge in the graph, we can simulate his removal, compute the final total flow and save it. At the end, we return the edge that causes the maximum flow.
...
Then we can track the evolution of computation time in function of the size if the random generated graph (\figref{benchmark_2_all}). On the normal scale, we can see how efficient the optimized method is comparing to the naive method. As the graph's size are all in the same order, the cardinal of the min-cut is similar which explains the low-growing function that we observe.
...
Finally, we will continue to use generated random graph to compare both methods. On the one hand, we can follow the evolution of the computational time with the size of the graph (\figref{benchmark_3a}). On the other compare, we can study how the greedy method is in fact close to the optimal solution (\figref{benchmark_3b}).
...
\begin{figure}[H]
    ...
    \end{subfigure}
    \caption{\textbf{Comparison between the greedy and the linear programming.} The \figref{benchmark_3a} present the execution time with the number of nodes in the graph and the \figref{benchmark_3b} present efficiency of the greedy algorithm comparing with the real optimal solution.}
