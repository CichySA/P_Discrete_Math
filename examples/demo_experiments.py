...
def demo_section6_3d_visualizations():
    ...
    from example_flow_visualizations import (
        ...
        animate_3d_saddle_point,
        draw_residual_capacity_surface,
        draw_flow_vector_field,
        draw_flow_routes_sankey,
    )

    G, pos = demo_network()
    flow_val, flow_dict = nx.maximum_flow(G, "s", "t", capacity="capacity")

    # 6a: 3D Feasible Flow Polytope
    print("[6a] 3D Feasible Flow Polytope")
    print("     Shows the actual 3D polytope for a 3-edge network,")
    print("     with the conservation plane f_e1=f_e2 and optimum marked.")
    draw_3d_feasible_polytope()
    plt.show()

    # 6b: 3D Feasible Region as Half-Space Intersection
    print("\n[6b] 3D Feasible Region — Half-Space Intersection")
    print("     Each capacity constraint is a half-space bounding plane.")
    print("     The feasible polytope is their intersection.")
    draw_feasible_region_3d_halfspaces()
    plt.show()

    # 6c: Stationarity / KKT Geometry
    print("\n[6c] Stationarity — KKT Condition ∇L = 0")
    print("     Left: primal objective + Lagrangian at λ*")
    print("     Right: stationarity residual |1/f − λ| heatmap")
    draw_stationarity_visualization()
    plt.show()

    # 6d: 3D Animated Saddle Point (view-only rotation)
    print("\n[6d] 3D Animated Saddle Point (rotating view)")
    print("     Camera orbits the Lagrangian surface, revealing the saddle.")
    ani_saddle = animate_3d_saddle_point(n_frames=90)
    html_saddle = ani_saddle.to_jshtml()
    plt.close("all")
    display(HTML(html_saddle))

    # 6e: Residual Capacity 3D Surface
    print("\n[6e] Residual Capacity 3D Surface")
    print("     Total flow achievable to sink given fixed source edge flows.")
    print("     Computed via LP solve at each grid point (25×25 = 625 solves).")
    draw_residual_capacity_surface(G)
    plt.show()

    # 6f: Flow Vector Field
    print("\n[6f] Flow Vector Field — Arrows scaled by flow magnitude")
    draw_flow_vector_field(G, flow_dict, pos)
    plt.show()

    # 6g: Flow Route Sankey
    print("\n[6g] Flow Route Decomposition (Sankey-style)")
    print("     Greedy path extraction decomposes flow into s→t routes.")
    draw_flow_routes_sankey(G, flow_dict)
    plt.show()

    print("\n=== All 3D & advanced visualizations complete ===")


# ═══════════════════════════════════════════════════════════════════════════════
# 7. KKT & Unimodularity — Why Relaxed Dual Stays Integral
# ═══════════════════════════════════════════════════════════════════════════════

def _build_demo_network_kkt():
    """Build the Wikipedia-style demo network for KKT/unimodularity demos."""
    G = nx.DiGraph()
    arcs = [
        ("s", "a", 8), ("s", "b", 4),
        ("a", "b", 6), ("a", "c", 4),
...
def _solve_relaxed_dual_lp(G, source="s", sink="t"):
    """
    Solve the relaxed dual LP (continuous d ∈ [0,1]).
    ...
    Returns (d_values, mu_values, cut_value).
    """
    edges = list(G.edges())
    nodes = list(G.nodes())
    n_edges = len(edges)
    cap = np.array([G[u][v]["capacity"] for u, v in edges], dtype=float)
    A_full = np.asarray(nx.incidence_matrix(G, oriented=True, dtype=float).todense())
    ...
    return d.value, mu.value, float(cap @ d.value)


def _solve_boolean_dual_milp(G, source="s", sink="t"):
    """
    Solve the boolean dual MILP.

    Returns (d_values, cut_value).
    """
    edges = list(G.edges())
    nodes = list(G.nodes())
    n_edges = len(edges)
    ...
    d_bool = cp.Variable(n_edges, boolean=True)
    mu_bool = cp.Variable(A_int.shape[0])
    constraints_bool = [A_int.T @ mu_bool + d_bool >= q]
    prob_bool = cp.Problem(cp.Minimize(cap @ d_bool), constraints_bool)
    prob_bool.solve(verbose=False)
    return d_bool.value, float(cap @ d_bool.value)
...
def demo_section7_kkt_unimodular():
    ...
    from example_flow_visualizations import (
        ...
        draw_random_capacity_integrality,
        draw_kkt_dual_analysis,
    )

    G, pos = _build_demo_network_kkt()
    edges = list(G.edges())
    n_edges = len(edges)
    edge_labels = [f"{u}→{v}" for u, v in edges]

    # 7a: Solve relaxed dual LP
    print("[7a] Solving RELAXED dual LP (continuous d ∈ [0,1])")
    d_relaxed, mu_relaxed, cut_relaxed = _solve_relaxed_dual_lp(G)
    print(f"     Cut value (relaxed): {cut_relaxed:.4f}")
    print("     Relaxed d values:")
    for j, (u, v) in enumerate(edges):
        is_int = abs(d_relaxed[j] - 0) < 1e-6 or abs(d_relaxed[j] - 1) < 1e-6
        print(f"       {u}→{v}: d={d_relaxed[j]:.6f}  "
              f"{'✓ INTEGRAL' if is_int else '✗ NON-INTEGRAL'}")

    # 7b: Solve boolean MILP for comparison
    print("\n[7b] Solving BOOLEAN MILP (d ∈ {0,1})")
    d_bool, cut_bool = _solve_boolean_dual_milp(G)
    print(f"     Cut value (boolean): {cut_bool:.4f}")
    print(f"     Match: {abs(cut_relaxed - cut_bool) < 1e-6}")

    # 7c: Visualize relaxed LP integrality
    print("\n[7c] Relaxed LP integrality comparison")
    draw_relaxed_dual_integrality(G, d_relaxed, d_bool)
    plt.show()

    # 7d: Total unimodularity heatmap + determinant check
    print("\n[7d] Total unimodularity of incidence matrix")
    draw_unimodularity_heatmap(G)
    plt.show()

    # 7e: KKT force analysis
    print("\n[7e] KKT conditions — why d is forced to {0,1}")
    draw_kkt_dual_analysis(G)
    plt.show()

    # 7f: Random capacity perturbation test
    print("\n[7f] Random capacity perturbation — integrality under perturbation")
    draw_random_capacity_integrality(G)
    plt.show()

    # 7g: Test on larger random DAG
    print("\n[7g] Testing integrality on random larger DAG (12 nodes, ~25 edges)")
    np.random.seed(77)
    G_big = nx.DiGraph()
    n_big = 12
    nodes_big = ["s"] + [f"v{i}" for i in range(n_big - 2)] + ["t"]
    # Create random DAG with s → ... → t structure
    for i in range(n_big - 1):
        G_big.add_edge(nodes_big[i], nodes_big[i + 1],
                       capacity=np.random.randint(3, 20))
    # Add random cross edges
    for _ in range(15):
        i, j = np.random.choice(range(n_big), 2, replace=False)
        if i < j and not G_big.has_edge(nodes_big[i], nodes_big[j]):
            G_big.add_edge(nodes_big[i], nodes_big[j],
                           capacity=np.random.randint(1, 15))

    d_big, mu_big, cut_big = _solve_relaxed_dual_lp(G_big)
    n_nonint_big = sum(not (abs(v - 0) < 1e-6 or abs(v - 1) < 1e-6)
                       for v in d_big)
    edges_big = list(G_big.edges())
    print(f"     {len(edges_big)} edges, {n_big} nodes")
    print(f"     Non-integral d: {n_nonint_big} / {len(edges_big)}")
    print(f"     Cut value: {cut_big:.4f}")
    if n_nonint_big > 0:
        print("     Non-integral edges:")
        for j, (u, v) in enumerate(edges_big):
            if not (abs(d_big[j] - 0) < 1e-6 or abs(d_big[j] - 1) < 1e-6):
                print(f"       {u}→{v}: d={d_big[j]:.6f}")
    else:
        print("     ✓ ALL integral — total unimodularity confirmed!")

    print("\n=== KKT & Unimodularity experiments complete ===")


# ═══════════════════════════════════════════════════════════════════════════════
# 8. KKT Force Balance — 2D/3D constraint-plane geometry
# ═══════════════════════════════════════════════════════════════════════════════
...
def demo_section8_kkt_force_balance():
    ...
    from example_flow_visualizations import (
        ...
        draw_kkt_constraint_polytope_3d,
        draw_kkt_force_field_across_polytope,
    )

    G, pos = _build_demo_network_kkt()

    # 8a: 1D KKT force balance (three cases)
    print("[8a] KKT Force Balance in 1D — three cases")
    print("     Left:   L(μ*) ≤ 0  → d* = 0  (wall d≥0 active)")
    print("     Center: L(μ*) ∈ (0,1) → THEORETICALLY possible, IMPOSSIBLE by TU")
    print("     Right:  L(μ*) ≥ 1  → d* = 1  (wall d≤1 active)")
    draw_kkt_force_balance_2d()
    plt.show()

    # 8b: 2D constraint-plane force diagram
    print("\n[8b] 2D Constraint-Plane Force Diagram")
    print("     Shows feasible polytope, objective contours, ∇f,")
    print("     and active constraint normals at the {0,1}² optimum.")
    draw_kkt_constraint_planes_2d(G)
    plt.show()

    # 8c: 3D constraint-force polytope
    print("\n[8c] 3D Constraint-Force Polytope")
    print("     Unit cube [0,1]³, coupling half-spaces, ∇f quiver,")
    print("     active normal cone at the {0,1}³ optimum vertex.")
    draw_kkt_constraint_polytope_3d(G)
    plt.show()

    # 8d: Gradient force field across the polytope
    print("\n[8d] KKT Gradient Force Field")
    print("     Sampling across the 2D polytope: every interior point is")
    print("     pushed by −∇(cᵀd) toward a {0,1}² vertex.")
    print("     Color = nearest {0,1}² vertex.")
    draw_kkt_force_field_across_polytope(G)
    plt.show()

    print("\n=== KKT Force Balance experiments complete ===")
